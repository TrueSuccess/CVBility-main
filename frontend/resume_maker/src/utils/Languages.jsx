...
export const LanguageComponents = ({id}) => {
    ...
    const { handleNewField,CurrentEntity, setCurrentEntity, LanProfData, setLanProfData,SelectedId,LanguageSettingInfoGraphic, LanProfCompArray, setLanProfCompArray,
    FontSize ,setSelectedId}= useContext(ComponentListContext)
    ...
    useEffect(() => {
        ...
        if(+id>0&&SelectedId==id&&CurrentEntity==='lan'){
            const underliend=document.getElementsByClassName(`${id}lang`)[0].querySelectorAll('.x')
            addLan.current.classList.remove('hidden')
                ...
                const notunderliend=document.getElementsByClassName(`${id}lang`)[0].querySelectorAll('.input-underline')
        ...
        // Premium3
        if (LanguageSettingInfoGraphic === 'Premium3' || LanguageSettingInfoGraphic === 'Premium4' ||LanguageSettingInfoGraphic === 'Premium1') {
            const div = document.getElementsByClassName('LanInfoDiv')
    ...
    const newData = { id, title };
        ...
        let existingObjectIndex = LanProfData.findIndex(object => object.id === id);
...
export default function Languages() {
    ...
    return (
        <div className='pb-3 pr-5 ' key='language'>
                ...
                {Icon && <span className=''><FontAwesomeIcon className='text-2xl ml-3' icon={faGlobe}></FontAwesomeIcon></span>
                    }
                     <div suppressContentEditableWarning={true} contentEditable="true" className={`text-${ThemeColor} notEditable text-[32px] font-[600] uppercase outline-none w-[400px] `} >LANGUAGE</div>
